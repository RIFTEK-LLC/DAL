...
{
    if (rf::mpack_write_str(&buf->data, &buf->size, key->data, key->size) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_bool(void* ctx, dalBlob_t* buf, bool value)
{
    if (rf::mpack_write_bool(&buf->data, &buf->size, value) != MPACK_OK)	return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_uint(void* ctx, dalBlob_t* buf, uint64_t value)
{
    if (rf::mpack_write_uint(&buf->data, &buf->size, value) != MPACK_OK)	return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_int(void* ctx, dalBlob_t* buf, int64_t value)
{
    if (rf::mpack_write_int(&buf->data, &buf->size, value) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_dbl(void* ctx, dalBlob_t* buf, double value)
{
    if (rf::mpack_write_dbl(&buf->data, &buf->size, value) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_str(void* ctx, dalBlob_t* buf, dalStr_t* value)
{
    if (rf::mpack_write_str(&buf->data, &buf->size, value->data, value->size) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_blob(void* ctx, dalBlob_t* buf, dalBlob_t* value)
{
    if (rf::mpack_write_blob(&buf->data, &buf->size, value->data, value->size) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};

dalResult_e	dal_to_mpack_write_obj_beg(void* ctx, dalBlob_t* buf, uint32_t count)
{
    if (rf::mpack_write_map_beg(&buf->data, &buf->size, count) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};
...
{
    if (rf::mpack_write_arr_beg(&buf->data, &buf->size, count) != MPACK_OK)		return DAL_MEM_ERR;
	return DAL_OK;
};
