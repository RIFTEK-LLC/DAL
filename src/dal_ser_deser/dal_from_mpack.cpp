...
dalResult_e	mpack_to_dal_read(void* ctx, uint8_t** buf, uint32_t* available, dalDeserializerValue_t* value)
{
    ...
    uint8_t			type		= rf::mpack_read_u8(buf, available);
	...
	switch (type)
	{
		...
		case 0xc4:
			if (*available < sizeof(uint8_t))	return DAL_FORMAT_ERR;
            value->size		= rf::mpack_read_u8(buf, available);
			if (*available < value->size)		return DAL_FORMAT_ERR;
			value->type		= DVT_BLOB;
			value->as_blob	= *buf;
			*buf			+= value->size;
		...
		case 0xc5:
            ...
            value->size		= rf::mpack_read_u16(buf, available);
			if (*available < value->size)		return DAL_FORMAT_ERR;
			value->type		= DVT_BLOB;
			value->as_blob	= *buf;
			*buf			+= value->size;
		...
		case 0xc6:
            ...
            value->size		= rf::mpack_read_u32(buf, available);
			if (*available < value->size)		return DAL_FORMAT_ERR;
			value->type		= DVT_BLOB;
			value->as_blob	= *buf;
			*buf			+= value->size;
		...
		case 0xca:
			...
			value->type		= DVT_DBL;
            value->as_dbl	= rf::mpack_read_float(buf, available);
			break;
		...
		case 0xcb:
			...
			value->type		= DVT_DBL;
            value->as_dbl	= rf::mpack_read_double(buf, available);
			break;
		...
		case 0xcc:
			if (*available < sizeof(uint8_t))	return DAL_FORMAT_ERR;
			value->type		= DVT_UINT;
            value->as_uint	= rf::mpack_read_u8(buf, available);
			break;
		...
		case 0xcd:
			if (*available < sizeof(uint16_t))	return DAL_FORMAT_ERR;
			value->type		= DVT_UINT;
            value->as_uint	= rf::mpack_read_u16(buf, available);
			break;
		...
		case 0xce:
			if (*available < sizeof(uint32_t))	return DAL_FORMAT_ERR;
			value->type		= DVT_UINT;
            value->as_uint	= rf::mpack_read_u32(buf, available);
			break;
		...
		case 0xcf:
			...
			value->type		= DVT_UINT;
            value->as_uint	= rf::mpack_read_u64(buf, available);
			break;
		...
		case 0xd0:
			...
			value->type		= DVT_INT;
            value->as_int	= rf::mpack_read_i8(buf, available);
			break;
		...
		case 0xd1:
			...
			value->type		= DVT_INT;
            value->as_int	= rf::mpack_read_i16(buf, available);
			break;
		...
		case 0xd2:
			...
			value->type		= DVT_INT;
            value->as_int	= rf::mpack_read_i32(buf, available);
			break;
		...
		case 0xd3:
			...
			value->type		= DVT_INT;
            value->as_int	= rf::mpack_read_i64(buf, available);
			break;
